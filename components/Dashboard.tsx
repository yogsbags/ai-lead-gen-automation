    ...
    <div className="flex h-screen bg-white overflow-hidden font-sans">
      ...
      <aside className="w-72 border-r flex flex-col z-20 bg-white">
        ...
        <nav className="flex-1 p-6 space-y-2">
          <SidebarItem icon="fa-th-large" label="Dashboard" active />
          <SidebarItem icon="fa-brain" label="My ICPs" />
          <SidebarItem icon="fa-users" label="Leads Table" badge={displayLeads.length.toString()} />
          <SidebarItem icon="fa-rss" label="Signals" badge="12" />
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-[#fafafa]">
        ...
        <header className="h-24 bg-white border-b px-10 flex items-center justify-between sticky top-0 z-10 shadow-sm">
          <div className="relative w-full max-w-2xl group">
            <input 
              type="text" 
              placeholder="Search deep profiles..." 
        ...
        <div className="flex-1 overflow-y-auto p-10 space-y-10">
          ...
          {/* Table Container */}
          <div className="bg-white border border-[#eeeeee] rounded-[2.5rem] shadow-xl shadow-gray-200/40 overflow-hidden">
...
const SidebarItem: React.FC<{ icon: string; label: string; active?: boolean; badge?: string }> = ({ icon, label, active, badge }) => (
  <a href="#" className={`flex items-center justify-between px-6 py-4 rounded-2xl text-sm font-bold transition-all ${active ? 'bg-[#f0f7ff] text-[#0070f3] shadow-inner' : 'text-gray-500 hover:bg-[#fafafa] hover:text-gray-900'}`}>
